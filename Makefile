SRCS	= 	ft_putchar.c ft_string.c ft_unsigned.c ft_putnbr.c ft_hex.c ft_address.c ft_printf.c

OBJS	= ${SRCS:.c=.o} 

SRCS_LIBFT    := ${addprefix libft/, ft_isalpha.c ft_isdigit.c ft_isalnum.c ft_isascii.c \
       ft_isprint.c ft_strlen.c ft_memset.c ft_bzero.c \
       ft_memcpy.c ft_memmove.c ft_strlcpy.c ft_strlcat.c \
       ft_calloc.c ft_strdup.c ft_toupper.c ft_tolower.c \
       ft_strchr.c ft_strncmp.c ft_memchr.c ft_memcmp.c \
       ft_strnstr.c ft_atoi.c ft_strrchr.c ft_substr.c \
       ft_strjoin.c ft_strtrim.c ft_split.c ft_itoa.c \
       ft_strmapi.c ft_striteri.c ft_putstr_fd.c ft_putchar_fd.c \
       ft_putendl_fd.c ft_lstnew.c ft_lstadd_front.c ft_lstsize.c \
       ft_lstlast.c ft_lstadd_back.c ft_lstdelone.c ft_lstclear.c \
       ft_lstiter.c ft_lstmap.c }

OBJS_LIBFT	= ${SRCS_LIBFT:.c=.o}

CC	= cc
RM	= rm -f
CFLAGS	= -Wall -Wextra -Werror
AR	= ar rcs

HEADER	= ft_printf.h

NAME	= libftprintf.a

all:	${NAME}

${NAME}:	${OBJS}  ${OBJS_LIBFT} 
	${AR} ${NAME} ${OBJS} ${OBJS_LIBFT}

.c.o:
	${CC} ${CFLAGS} -o $@ -c $<

clean:
	${RM} ${OBJS} ${OBJS_bonus} ${OBJS_LIBFT}

fclean:	clean
		${RM} ${NAME}

re:	fclean all

.PHONY:	all clean fclean re
